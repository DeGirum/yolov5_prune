...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Mehrdad
        ...
        for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
            ...
            if RANK in {-1, 0}:
                # mAP
                callbacks.run('on_train_epoch_end', epoch=lth_epoch)
                ...
                callbacks.run('on_fit_epoch_end', log_vals, lth_epoch, best_fitness, fi)
                ...
                if (not nosave) or (final_epoch and not evolve):  # if save
                    ...
                    last, best = w / 'last{}.pt'.format(lth_stage), w / 'best{}.pt'.format(lth_stage)
                    torch.save(ckpt, last)
                    if best_fitness == fi:
                        torch.save(ckpt, best)
                    if lth_epoch > 0 and opt.save_period > 0 and lth_epoch % opt.save_period == 0:
                        torch.save(ckpt, w / f'epoch{lth_epoch}.pt')
                    del ckpt
